...
char *intToString(void *x) {
    int *num = (int *)x;
    char *str = (char *)malloc(12 * sizeof(char)); //memory allocation for string
    if (str == NULL) {
        printf ("Memory allocation failure\n");
        exit(EXIT_FAILURE);
    }
    snprintf(str, 12, "%d", *num); //integer to string 
    return str;
}
...
char *account_toString(void *v) {
    ...
    char *str = (char *)malloc(strlen(acc->name) + 15); //memory allocation for string
    if (str == NULL) {
        printf("Memory allocation failure\n");
        exit(EXIT_FAILURE);
    }
    snprintf(str, strlen(acc->name) + 15, "%s: %d", acc->name, acc->balance); //string formatting
    return str;
}
