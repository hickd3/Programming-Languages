...
// function prototype for markHeapChunk
void markHeapChunk(ProgramState *state, HeapChunk *chunk);
...
void markAndSweep(ProgramState *state) {
    ...
    //trace throught the heap chunks found in the stack
    for (int i = 0; i < state->num_vars_on_stack; i++) {
        if (state->stack[i].reference != NULL) {
            // Perform DFS to mark reachable chunks
            markHeapChunk(state, state->stack[i].reference);
        }
    }
    ...
    printf("Heap chunks:\n");
    for (int i = 0; i < state->num_heap_chunks; i++) {
        if (state->heap[i]->marked == 0) {
            printf("Heap chunk is at %p\n", (void *)state->heap[i]);
        }
    }
}


//depth-first search marks chunks in reach
void markHeapChunk(ProgramState *state, HeapChunk *chunk) {
    if (chunk == NULL || chunk->marked == 1) {
        return;  // dont enter an infinite loop or repeat marked chunks
    }
    ...
    for (int i = 0; i < chunk->num_references; i++) {
        markHeapChunk(state, chunk->references[i]);
    }
}
...
//citing help from Lucy Barest on workign through this code example with me
    HeapChunk *aChunk = HeapMalloc(state);
    setVar(state, "a", aChunk);  // a = malloc().
    HeapChunk *bChunk = HeapMalloc(state);
    setVar(state, "b", bChunk);    // b = malloc()
    HeapChunk *cChunk = HeapMalloc(state);
    cChunk = HeapMalloc(state);
    setVar(state, "c", cChunk);  // z = malloc()
    addReference(aChunk, bChunk);  // a->ref = b
    addReference(bChunk, cChunk); // b->ref = c
