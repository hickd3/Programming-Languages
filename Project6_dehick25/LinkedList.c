...
/* This file has been updated and differs from project5*/
...
LinkedList *ll_create(){
    ...
    if (list == NULL) {
        printf("Error: Memory allocation failed for LinkedList.\n");
        return NULL;
    }
    list->size = 0;
    list->head = NULL;
...
void ll_push(LinkedList *l, void *data){
    Node* newNode = malloc(sizeof(Node)); //find space for a new node
    newNode->value = data; //newNode gets the data parameter as its value
...
void *ll_find(LinkedList *l, void *target, int (*compfunc)(void *, void *)) {
    ...
    while (current != NULL) {
        if (compfunc(current->value, target)) {
...
void ll_clear(LinkedList *l) {
    if (l == NULL) {
        printf("Encounterd NULL pointer for LinkedList.\n");
        return;
    }
    Node *current = l->head;
    while (current != NULL) {
        Node *temp = current;
        current = current->next;
        free(current->value); //free the node value
        free(temp); //free the node memory allocation
    }
